...
namespace System.Common.References
{
  ...
  #region InvalidSourceListException
  ...
  public class IncludeAllItemFilter<T> : IItemFilter<T>
  {
    ...
    private static Lazy<IncludeAllItemFilter<T>> _instance = new Lazy<IncludeAllItemFilter<T>>(true);
  ...
  #region INotifyingEditableObject
  ...
  #region AggregateBindingListView
  ...
  {
    #region MultiSourceIndexList

    protected class MultiSourceIndexList<TValue> : List<KeyValuePair<ListItemPair<TValue>, int>>
    {
      public void Add(IList sourceList, ObjectView<TValue> item, int index)
      {
        Add(new KeyValuePair<ListItemPair<TValue>, int>(new ListItemPair<TValue>(sourceList, item), index));
      }
      ...
      /// </summary>
      /// <param name="item">The <typeparamref name="TValue"/> item to search for.</param>
      /// <returns>Returns the index in this list of the item, or -1 if not found.</returns>
      public int IndexOfItem(TValue item)
      {
        for (int i = 0; i < Count; i++)
        {
      ...
      /// <returns>Returns the index in this list of the item, or -1 if not found.</returns>
      public int IndexOfKey(ObjectView<TValue> item)
      {
        for (int i = 0; i < Count; i++)
        {
      ...
      /// </summary>
      /// <param name="item">The <typeparamref name="TValue"/> item to check for.</param>
      ...
      public bool ContainsKey(ObjectView<TValue> key)
      {
      ...
      /// </summary>
      public ObjectView<TValue>[] Keys
      {
        get
        {
          return ConvertAll<ObjectView<TValue>>(new Converter<KeyValuePair<ListItemPair<TValue>, int>, ObjectView<TValue>>(
            delegate(KeyValuePair<ListItemPair<TValue>, int> kvp)
            { return kvp.Key.Item; }
            )).ToArray();
        }
      }
      ...
      public IEnumerator<ObjectView<TValue>> GetKeyEnumerator()
      {
        foreach (KeyValuePair<ListItemPair<TValue>, int> kvp in this)
        {
    ...
    protected class ListItemPair<TValue>
    {
      private IList _list;
      private ObjectView<TValue> _item;

      public ListItemPair(IList list, ObjectView<TValue> item)
      {
        _list = list;
        _item = item;
      }
    ...
    #region ProvidedViewPropertyDescriptor

    protected class ProvidedViewPropertyDescriptor<TValue> : PropertyDescriptor
    {
      ...
      {
        get { return typeof(ObjectView<TValue>); }
      }
      ...
      public override object GetValue(object component)
      {
        ...
        {
          return (component as ObjectView<TValue>).GetProvidedView(Name);
        }
    ...
    public IList SourceLists
    {
      ...
      set
      {
        if (value == null)
        {
          throw new ArgumentNullException("SourceLists", "SourceLists cannot be null.");
        }
        ...
        foreach (object obj in value)
        {
          ...
          foreach (object item in list)
          {
            if (!(item is T))
            {
              throw new InvalidSourceListException(string.Format("Item in list is not of type {0}.", typeof(T).FullName));
            }
          }
        }
    ...
    protected virtual T OnAddingNew()
    {
      ...
      {
        throw new InvalidOperationException("Cannot add a new item due to no object being provided in the AddingNew event and a lack of default public constructor.");
      }
    }
    ...
    public IList NewItemsList
    {
      ...
      set
      {
        ...
        {
          throw new ArgumentException("Source list is not in the view.");
        }
        _newItemsList = value;
      }
    }
  ...
  */
    ...
    int IList.Add(object value)
    {
      ...
      throw new NotSupportedException("Cannot add an external item to the view. Use AddNew() or add to source list instead.");
    }
    ...
    protected internal object CreateProvidedView(ObjectView<T> obj, PropertyDescriptor sourceListProperty)
    {
      object list = sourceListProperty.GetValue(obj);
      Type viewType = GetProvidedViewType(sourceListProperty);

      //return Activator.CreateInstance(viewType, list);

      ConstructorInfo info = viewType.GetConstructor(new Type[] { sourceListProperty.PropertyType });
      return info.Invoke(new object[] { list });
    }
