  ...
  public class EnumWindowsItem
  {
    ...
    public Image Snapshot
    {
      get
      {
        ...
        // update the window
        UnManagedMethods.UpdateWindow(this.Handle);

        // wait
        Thread.Sleep(500);

        // get te hDC of the target window
        IntPtr hdcSrc = UnManagedMethods.GetWindowDC(this.Handle);

        // get the size
        RECT windowRect = new RECT();
        UnManagedMethods.GetWindowRect(this.Handle, ref windowRect);
        ...
        IntPtr hdcDest = UnManagedMethods.CreateCompatibleDC(hdcSrc);

        // create a bitmap we can copy it to,
        // using GetDeviceCaps to get the width/height
        IntPtr hBitmap = UnManagedMethods.CreateCompatibleBitmap(hdcSrc, width, height);

        // select the bitmap object
        IntPtr hOld = UnManagedMethods.SelectObject(hdcDest, hBitmap);

        // bitblt over
        UnManagedMethods.BitBlt(hdcDest, 0, 0, width, height, hdcSrc, 0, 0, UnManagedMethods.SRCCOPY);

        // restore selection
        UnManagedMethods.SelectObject(hdcDest, hOld);

        // clean up 
        UnManagedMethods.DeleteDC(hdcDest);
        UnManagedMethods.ReleaseDC(this.Handle, hdcSrc);
